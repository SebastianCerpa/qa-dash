...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!payload.testSuiteId || !payload.testCaseName || !payload.status) {
    ...
    return NextResponse.json({ 
      ...
      createdBugId: testExecution.created_bug_id
    });
  } catch (error) {
...
async function createBugFromFailedTest(testExecution: any, payload: any) {
  try {
  ...
  try {
    ...
    for (const lead of qaLeads) {
      await prisma.notifications.create({
        data: {
          user_id: lead.id,
          message: `⚠️ Flaky Test Detected: ${testCaseName} (${Math.round(failureRate * 100)}% failure rate)`,
          is_read: false
        }
      });
    }
...
async function updateTestAnalytics(payload: any) {
  try {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    // Update or create daily analytics
    const analytics = await prisma.bug_analytics.upsert({
      where: { date: today },
      update: {},
      create: {
        date: today,
